...
void sf_delete_tree_structure1(sfnode current_node)
{
    if(current_node == NULL)
        return;
    sfnode* current_child_ptr = current_node->children;
    ...
    if(current_node->child == NULL && current_child_ptr != NULL)
    {
        for(idx = 0; idx < last_index(current_node->data_item); idx++)
        {
            if(current_child_ptr[idx] != NULL)
            {
                ...
                sf_delete_tree_structure1(this_child);
                sf_delete_tree_structure1(this_child->child);
                sf_delete_tree_structure1(this_child->next);
                free(this_child); /* let the child go*/
                this_child = NULL;
...
void sf_create_and_insert_new_child(sfnode current_node, data d, int tid)
{
    ...
    int idx = index((data_type) data_item, current_node->data_item);
    ...
    while(qstack->size > 0) /* we go on till the time we have nodes in the stack*/
    {
        ...
        else
        {
            ...
            while(itemlen)
            {
                ...
                */
                sf_create_update_header_node(htable, temp_data, root_data, tid);
                idx = index(temp_data, root_data); /* idx in the header table,\
...
void sf_insert_itemset(sforest forest, data d, int tid)
{
    ...
    while(old_pos < DICT_SIZE && bit_array_find_next_set_bit(d, old_pos, &res))
    {
        ...
        if(size == 0) /* res is the last item*/
        {
            tree->head_table[index(res, tree->head_table[0]->data_item)]->cnt++;
            return;
        }
        sf_append_buffer(tree->root, d, 1, tid); /* transaction: acdef, node a will have 'cdef'*/
...
int sf_print_patterns_to_file(int* collected, buffer buff, double cnt, int end, int pattern)
{
    ...
    while(buff)
    {
        ...
        while(bit_array_find_next_set_bit(temp, old_pos, &data_item) == 0)
        {
...
void sf_fp_mine_frequent_itemsets(sftree tree, data_type sorted, data till_now, sfnode collected, int tid, double minsup)
{
    ...
    {
        sftree cond_tree = sf_create_conditional_sf_tree(tree, curr_data,
                           minsup, tid);
...
void sf_prune_buffer1(sfnode curr, int tid)
{
