...
void sf_append_buffer(sfnode curr, data d, double freq, int tid)
{
    ...
    curr_buffer->freq *= pow(DECAY, get_currtime() - curr_buffer->ltid);
    ...
    curr_buffer->ltid = get_currtime();

    /* update the frequency of buffer items */
    while(head)
    {        
        if(sf_is_equal(head->itemset, d))
        {
            currtime = get_currtime();
            head->freq *= pow(DECAY, currtime - head->ltid);
            head->ltid = currtime;
            head->freq += freq;
            return;
        }
...
buffer sf_get_buffer(sfnode curr, int bucket, int tid)
{
    ...
    if(curr_buffer->bufferhead)
        curr_buffer->bufferhead->prev = NULL; // first element has NULL 'prev'
    currtime = get_currtime();
...
buffer sf_pop_buffer(sfnode curr, int bucket, int tid)
{
    ...
    /* updating the outgoing buffer here only*/
    to_pop->freq *= pow(DECAY, currtime - to_pop->ltid);
    to_pop->ltid = get_currtime();
    to_pop->next = NULL;
...
void sf_fp_insert(fpnode current_node, header_table* htable, data d, double cnt, int tid)
{
    ...
    currtime = get_currtime();
    curr_header_node->cnt *= pow(DECAY, currtime - curr_header_node->ltid);
    curr_header_node->ltid = currtime;
    ...
    currtime = get_currtime();
    current_node->freq *= pow(DECAY, currtime - current_node->ltid);
    current_node->freq += cnt;
    current_node->ltid = currtime;
...
int sf_insert_itemset_helper(sfnode node, int root_data, int tid, double total_time, timeval* start)
{
    ...
    data temp, d;
    double toss, elapsedTime, currtime;
    int idx;
    ...
    while(qstack->size > 0) /* we go on till the time we have nodes in the stack*/
    {
        ...
        else
        {
            ...
            while(temp[1] > 0)
            {
                ...
                /* If popped is not frequent and it's corresponding is child absent then continue*/
                if(popped->freq < EPS*(tid - popped->ftid) && current_child_ptr[idx] == NULL)
                {
                ...
                /* updating the frequency of the node according to the formula*/
                currtime = get_currtime();
                current_child_ptr[idx]->freq *= pow(DECAY, currtime - current_child_ptr[idx]->ltid);
                assert(currtime - popped->ltid >= 0);
                // assert(popped->ltid == tid); /* updated in the pop function itself*/
                current_child_ptr[idx]->ltid = currtime;
            ...
            while(temp[1] > 1)
            {
                ...
                if(current_child_ptr[idx]) /* as the child might not have been inserted before*/
                {
                    ...
                    if(tid - current_child_ptr[idx]->last_pruned > BATCH*GAMMA*lvl/10)
                    {
                        ...
                        MIN_BUFFER_SIZE[lvl] = min(MIN_BUFFER_SIZE[lvl], old);
                        AVG_BUFFER_SIZE[lvl] += old;
                        RED_BUFFER_SIZE[lvl] += old - new;
                        CNT_BUFFER_SIZE[lvl]++;
                    }
...
int sf_insert_itemset_helper1(sfnode node, int root_data, int tid, double total_time, timeval* start)
{
    /* this functions just deletes the subtree after every insert. */
    /* currently node is the root node*/
    assert(node != NULL);
    ...
    while(qstack->size > 0) /* we go on till the time we have nodes in the stack*/
    {
        ...
        else
        {
            ...
            /* start is the timeval struct which tells whether we want to preempt */
            if(start)
            {
                gettimeofday(&curr, NULL);
                elapsedTime = (curr.tv_sec - start->tv_sec) * 1000.0;
...
void sf_insert_itemset(sforest forest, data d, int tid, double total_time, timeval* start)
{
    ...
    while(d[1] > 0) /* this is taking time as with higher avg. len we have to insert in many trees*/
    {
        ...
        assert(get_currtime() - root->ltid >= 0);
        root->freq *= pow(DECAY, get_currtime() - root->ltid);
        root->freq++;
        root->ltid = get_currtime();
...
sforest sf_merge_tree(sforest f1, sforest f2, int tid)
{
    ...
    /* it will store FIs in file named results_{STREAMS+1} */
    /* mining for frequent patterns */
    int no_patterns = sf_mine_frequent_itemsets(f2, 0, -2, STREAMS); 
    // sf_delete_sforest(f2);
...
int sf_mine_frequent_itemsets_helper(sfnode node, int* collected, int end, int tid, int pattern, int rank)
{
    ...
    if(node->freq >= prod)
    {
        ...
        if(end >= 0)
        {
            ...
            */
            cnt += sf_print_patterns_to_file(collected, collector->bufferhead, node->freq, end, pattern, tid, rank);
        ...
        for(idx = 0; idx < last_index(node->data_item); idx++)
        {
            ...
            if(this_child)
                cnt += sf_mine_frequent_itemsets_helper(this_child, collected, end, tid, pattern, rank);
        }
    }
    return cnt;
}
...
int sf_mine_frequent_itemsets(sforest forest, int tid, int pattern, int rank)
{
    ...
    for(idx = 0; idx < DICT_SIZE; idx++)
    {
        ...
        cnt += sf_mine_frequent_itemsets_helper(root, collected, -1, tid, pattern, rank);
...
fptree sf_create_conditional_fp_tree(fptree tree, data_type data_item, double minsup, int tid)
{
    ...
    if(tree->head_table[idx] == NULL)
    {
        ...
        tree->head_table[idx]->ltid = get_currtime();
    }
    ...
    if(curr_head_table_node == NULL || (curr_head_table_node->cnt)*\
       pow(DECAY, get_currtime() - curr_head_table_node->ltid) < minsup)
    {
        // printf("rejecting %d\n", data_item);
        return NULL;
    }
    ...
    {
        temp = node;
        add = (temp->freq)*pow(DECAY, get_currtime() - temp->ltid);
        while(temp != NULL)
        {
            temp->touched += add;
            // printf("touched = %lf\n", temp->touched);
...
void sf_fp_mine_frequent_itemsets(fptree tree, data_type sorted, data till_now, bufferTable collected, int tid, double minsup)
{
    ...
    if(till_now[1] > 0)
    {
        ...
        double new_cnt = (curr_header_node->cnt)*pow(DECAY, get_currtime() - curr_header_node->ltid);
...
void sf_empty_tree(sfnode root, int tid)
{
    ...
    {
        sf_insert_itemset_helper1(root, root_data, tid, -1, NULL);
    }
    ...
    else
    {
        while(qstack->size > 0)
        {
            ...
            for(idx = 0; idx < last; idx++)
            {
                ...
                {
                    sf_insert_itemset_helper1(current_node->children[idx], root_data, tid, -1, NULL);
                }
                else
...
void sf_fp_merge(fpnode parent, fpnode child, int tid)
{
    ...
    childptr->freq = childptr->freq * pow(DECAY, get_currtime() - childptr->ltid)\
                           + child->freq * pow(DECAY, get_currtime() - childptr->ltid);
    ...
    for(i = 0; i < HSIZE; i++) // iterating through all the buckets
    {
        ...
        if(curr->hbuffer[i])
        {
            ...
            while(curr_buff == head && head != NULL)
            {
                ...
                curr_buff->freq *= pow(DECAY, get_currtime() - curr_buff->ltid);
                curr_buff->ltid = get_currtime();
        ...
        for(child = 0; child < last_index(node->data_item); child++)
        {
            ...
            if(node->children[child])
            {
                node->children[child]->freq *=\
                        pow(DECAY, get_currtime() - node->children[child]->ltid);
                if(node->hbuffer[child%HSIZE])
                {
                    node->hbuffer[child%HSIZE]->freq *= pow(DECAY, get_currtime() - node->hbuffer[child%HSIZE]->ltid);
...
void sf_print_sfnode(sfnode node)
{
    ...
    if(node->parent)
        printf("data_item = %d, freq = %lf, ftid = %d, ltid = %lf, parent = %d, children = %d, buffer_size = %d \n", node->data_item, node->freq, node->ftid, node->ltid, node->parent->data_item, c, b);
    else
        printf("data_item = %d, freq = %lf, ftid = %d, ltid = %lf, parent = NULL, children = %d, buffer_size = %d \n", node->data_item, node->freq, node->ftid, node->ltid, c, b);
...
void sf_print_fpnode(fpnode node)
{
    ...
    if(node->parent)
        printf("data_item = %d, freq = %lf, ftid = %d, ltid = %lf, parent = %d, touched = %lf, children = %d\n", node->data_item, node->freq, node->ftid, node->ltid, node->parent->data_item, node->touched, c);
    else
        printf("data_item = %d, freq = %lf, ftid = %d, ltid = %lf, parent = NULL, touched = %lf, children = %d\n", node->data_item, node->freq, node->ftid, node->ltid, node->touched, c);
...
void sf_print_buffer_table(bufferTable* hbuffer)
{
    ...
    for(i = 0; i < HSIZE; i++)
    {
        if(hbuffer[i])
        {
            printf(">>>>>>> freq = %lf, tid = %lf, bucket = %d, collision = %c<<<<<<<\n",\
...
{
    printf("(ftid: %d, ltid: %lf); freq = %lf --> ", node.ftid, node.ltid, node.freq);
    sf_print_data_node(node.itemset);
}
...
void sf_print_buffer(buffer head)
{
    ...
    while(buff)
    {
        printf("(ftid: %d, ltid: %lf); freq = %lf --> ", buff->ftid, buff->ltid, buff->freq);
